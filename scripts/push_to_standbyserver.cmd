...
for /f "tokens=1-2 del ims=: " %%a in ('time /t') do set XTime=%%a.%%b
rem ********************************************************************************
...
REM  *********************************************************************************
ECHO ---- Verifying connection to Standby Server. ---- 

:pingA
ping wcloudb.schoolofone.net -n 1 -w 20000 > NUL 
...
:backprod
echo ---Moving previous DB backups into oldDBbackup [Standby] ---
ssh Administrator@wcloudb.schoolofone.net "cd /cygdrive/c/DB/DBbackups/ && mv *.bak /cygdrive/c/DB/DBbackups/oldDBbackups/"
if errorlevel 1 goto :backuperror_prod
echo ............................
echo.

:transfer
echo --- Transferring DB to Standby Server ---
cd c:\DB\dbBackups\
scp *.bak.gz Administrator@wcloudb.schoolofone.net:/cygdrive/c/DB/DBbackups/
if errorlevel 1 goto :transferror
echo ............................
echo.

:unzip
echo --- Decompressing DB on the Standy Server ---
ssh -i ~/.ssh/prod_key Administrator@wcloudb.schoolofone.net "cd /cygdrive/c/DB/DBbackups/ && gzip -dv *.bak.gz"
****decompress on standby
...
:erroconnect
echo No connection to the Standby Server.
exit 1
...
:backuperror_prod
echo Moving previous DB backups into oldDBbackup in Standby Server failed!
goto :transfer

:transferror
echo Error while tranferring DB backup from Dev. to the Standby.!
exit 1

:unziperror
echo Error while Decompressing file in Staging.!
exit 1
